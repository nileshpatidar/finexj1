...
DECLARE
  ...
  v_all_time_earnings NUMERIC(24, 4) := 0;
  v_all_time_rewards_total NUMERIC(24, 4) := 0;
...
BEGIN
  ...
  -- 7. All-Time Credited User Credits used by reconciliation.
  -- These credits are already present in the user ledger and therefore must be
  -- represented on the expected-position side of this internal ledger reconciliation.
  -- Earnings may be positive or negative; referral rewards are credited user funds.
  SELECT COALESCE(SUM(COALESCE(earnings_amount, 0)), 0)::NUMERIC(24, 4)
  INTO v_all_time_earnings
  FROM earnings
  WHERE status = 'credited';

  SELECT COALESCE(SUM(COALESCE(amount, 0)), 0)::NUMERIC(24, 4)
  INTO v_all_time_rewards_total
  FROM referral_rewards
  WHERE status = 'credited';
  ...
  -- Expected internal accounting position includes every user credit represented in the ledger:
  -- confirmed deposits + credited earnings (positive/negative) + credited referral rewards,
  -- plus operational inflows, less net payouts and operational outflows.
  -- This is intentionally an internal ledger reconciliation, not a claim about external
  -- trading-bank/custody assets. Unfunded external liabilities should remain reviewable.
  v_net_system_capital := (v_all_time_deposited + v_all_time_earnings + v_all_time_rewards_total + v_op_fund_inflow)
    - v_all_time_withdrawn_net - v_op_fund_outflow;
